...
describe('RotateUserGesture', () => {
  ...
  it('should perform a basic rotation with default options', async () => {
    ...
    const initialDistance = Math.sqrt(
      Math.pow(firstDownCall.clientX - secondDownCall.clientX, 2) +
        Math.pow(firstDownCall.clientY - secondDownCall.clientY, 2)
    );

    const finalDistance = Math.sqrt(
      Math.pow(lastMoveFirstPointer.clientX - lastMoveSecondPointer.clientX, 2) +
  ...
  it('should respect custom rotation angle', async () => {
    ...
    const displacement = Math.sqrt(
      Math.pow(lastMoveFirstPointer.clientX - firstDownCall.clientX, 2) +
